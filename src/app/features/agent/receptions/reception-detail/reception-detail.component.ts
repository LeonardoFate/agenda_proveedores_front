...
import { EstadoReserva, ReservaDetalle } from '../../../../core/models/reserva.model';
...
export class ReceptionDetailComponent implements OnInit {
  ...
  startReception(): void {
    ...
    this.reservationService.updateReservationStatus(this.reservationId, EstadoReserva.EN_RECEPCION).subscribe({
      next: () => {
        ...
        this.reservationService.startTimeRecord(
          ...
          next: () => {
            if (this.reservation) {
              this.reservation.estado = EstadoReserva.EN_RECEPCION;
            }
            this.loadTimeRecords();
          },
          error: (error) => {
    ...
    if (activeRecord) {
      ...
      this.reservationService.finishTimeRecord(activeRecord.id).subscribe({
        next: () => {
          ...
          this.reservationService.updateReservationStatus(this.reservationId, EstadoReserva.COMPLETADA).subscribe({
            next: () => {
              if (this.reservation) {
                this.reservation.estado = EstadoReserva.COMPLETADA;
              }
              this.loadTimeRecords();
            },
            error: (error) => {
